...
#if defined(WIN32)
BOOL WINAPIV DllMain(HMODULE hModule, DWORD ul_reason_for_call,
                     LPVOID lpReserved) {
...
extern "C" EXPORT enum Status CDECL Client__query(HNTP self,
                                                  const char* hostname,
...
extern "C" EXPORT enum Status CDECL Client__query_and_sync(
    HNTP self, const char* hostname, struct Result** result_out) {
  ResultEx* result = nullptr;
  *result_out = new Result();

  Status ret =
      reinterpret_cast<IClient*>(self)->query_and_sync(hostname, &result);
...
extern "C" EXPORT void CDECL Client__format_info_str(struct Result* result,
                                                     char* str_out) {
  Client::format_info_str(result, str_out);
}

extern "C" EXPORT void CDECL Client__get_status_str(enum Status status,
                                                    char* str_out) {
...
Client::~Client() { delete this->packet; }
...
Status Client::query(const char* hostname, ResultEx** result_out) {
  ...
  try {
    ...
    if (ret2 != Status::OK) return ret2;

    if (*result_out == nullptr) *result_out = new ResultEx();
    ...
    if (this->prev_delay > 0) {
      ...
      if (ringbuff_count < JITTER_WINDOW) ringbuff_count++;

      for (int i = 0; i < ringbuff_count; i++) sum += delay_ringbuff[i];
...
void Client::extract_time_point(time_point_t tp, int& y, int& m, int& d,
                                int& hr, int& min, int& sec, int& ms, int& us,
                                int& ns) noexcept {
  time_t tt =
...
void Client::format_info_str(Result* result, char* str) {
  if (result != nullptr) {
    ...
    sprintf(str, "%s%s%s%s%s", time_str, offset_str, delay_str, jitter_str,
            delta_str);
  }
}
...
void Client::get_status_str(Status status, char* str) {
  ...
  if (s >= 0 && s <= 8) strcpy(str, status_s[s]);
}
...
Status Client::init(const char* hostname) {
  ...
  if ((this->s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) == SOCKET_ERROR) {
    // printf(&quot; socket() failed with error code : % d & quot;,
    // WSAGetLastError());
  ...
  setsockopt(this->s, SOL_SOCKET, SO_RCVTIMEO, (const char*)&rx_timeout,
             sizeof(rx_timeout));
...
Status Client::query() {
  ...
  this->packet->precision =
      (uint8_t)-10;  // Precision of the local clock (expressed in power of 2,
                     // -10 means 2-10, that is to say 1/1024=0.97ms)
  this->packet->rootDelay = 256;  // Total round trip delay time
  this->packet->rootDispersion =
      256;  // Max error aloud from primary clock source
  ...
  if (sendto(this->s, this->msg, NTP_PACKET_LEN, 0,
             (struct sockaddr*)&this->si_addr, this->slen) == SOCKET_ERROR) {
    // printf(&quot; sendto() failed with error code : % d & quot;,
    // WSAGetLastError());
    return Status::SEND_MSG_ERR;
  }

  // receive message
  if (recvfrom(this->s, this->ans, NTP_PACKET_LEN, 0,
               (struct sockaddr*)&this->si_addr, &this->slen) == SOCKET_ERROR) {
    // printf(&quot; recvfrom() failed with error code : % d & quot;,
    // WSAGetLastError());
    if (WSAGetLastError() == WSAETIMEDOUT) return Status::RECEIVE_MSG_TIMEOUT;
    return Status::RECEIVE_MSG_ERR;
  }
  time_point_t t4 = std::chrono::time_point_cast<std::chrono::nanoseconds>(
      std::chrono::system_clock::now());
  ...
  t2_ns +=
      ((double)this->packet->txTm_f / 4294967295) * 1000000000;  // add fraction
  t3_ns += ((double)this->packet->rxTm_f / 4294967295) *
           1000000000;  // 0x100000000ULL = 4294967295

  double t1_ns = (double)std::chrono::duration_cast<std::chrono::nanoseconds>(
                     t1.time_since_epoch())
                     .count();  // nanoseconds from 1970
  double t4_ns = (double)std::chrono::duration_cast<std::chrono::nanoseconds>(
                     t4.time_since_epoch())
                     .count();

  double t1_b_ns = (double)t1_b.time_since_epoch()
                       .count();  // nanoseconds from start measuring
